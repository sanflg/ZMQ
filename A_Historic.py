...
"""
    ...
    for v2.0.1 in which a Client requests rate history from EURJPY Daily from 2019.01.04 to
    to 2019.01.14.
...
class rates_historic(Strategy):
    ...
    ##########################################################################
    '''
    from rates_historic import rates_historic
    a = rates_historic()
    a.run()
    '''
    ...
    def run(self):
        ...
        # request rates
        print('Requesting Daily Rates from EURJPY')
        self._zmq._DWX_MTX_SEND_HIST_REQUEST_(_symbol='EURJPY',
                                              _timeframe=1440,
                                              _start='2010.05.01 00:00:00',
                                              _end  ='2020.05.02 00:00:00')
        sleep(1)
        print('\nHistory Data Dictionary:')
        b = self._zmq._History_DB
        with open ('averprobemo.csv','w', newline = "") as f:
            w = csv.writer(f)
            w.writerow(b['EURJPY_D1'][0].keys())
            for i in b['EURJPY_D1']:
                w.writerow(i.values())
...
if __name__ == "__main__":
  ...
  # creates object with a predefined configuration: historic EURJPY_D1 between 4th adn 14th January 2019
